...
def analyze_chart(image_bytes):
    ...
    question = (
        ...
        "Тебе дан скриншот графика. При наличии — могут быть дополнительные данные (бары с ценами и объёмами, кластерные объёмы/POC, новости/фундаментальные события, потоки).\n\n"
        
        "Твоя задача — проанализировать график и найти торговые возможности. Выполни многофакторный анализ ВНУТРЕННЕ (не выводи промежуточные рассуждения) и СТРОГО сохрани формат ответа.\n"
        ...
        "АНАЛИЗИРУЙ ВНИМАТЕЛЬНО (внутренние шаги перед выводом):\n"
        "- Тренд и контекст по барам/свечам: структура тел/теней, импульсы/коррекции, волатильность (ATR/True Range), гэпы, уровни, объемы.\n"
        "- Объёмы: общий объём, всплески, кластера/POC, аномалии и согласованность с движением цены.\n"
        "- Технический анализ: мульти‑таймфрейм контекст, сила тренда/флет; скользящие (SMA/EMA/WMA), их угол и пересечения; осцилляторы (RSI/Stoch/MACD), дивергенции, зоны перекупленности/перепроданности; уровни S/R и паттерны (pin bar, engulfing, triangle, H&S и т.п.).\n"
        "- Поток ордеров/микроструктура: спред, лента принтов, айсберги, снятие ликвидности.\n"
        "- Риск‑менеджмент: волатильность, логичные зоны SL/TP, сценарии и уровни инвалидации.\n"
        "- Фундаментал: макро (ставки, CPI, NFP), корпоративные отчёты/гайды/байбэки, регуляторные риски, сезонность, позиционирование/потоки, сентимент.\n\n"
        
        "Строгие правила:\n"
        "- Анализируй только доступные данные: не выдумывай фундаментал или кластера, если их нет на входе.\n"
        "- Сигналы должны быть реалистичны для внутридневной торговли (удержание 1–2 часа).\n"
        "- Будь активным в поиске сигналов; No Trade — только при отсутствии чётких возможностей.\n"
        "- Stop Loss (SL) и Take Profit (TP) указывай логично по уровням/волатильности/ликвидности.\n"
        "- ОТВЕЧАЙ ТОЛЬКО В УКАЗАННОМ НИЖЕ ФОРМАТЕ: не меняй структуру, порядок, названия и типы полей, не добавляй/не удаляй поля, не меняй числовую точность и формат.\n\n"
